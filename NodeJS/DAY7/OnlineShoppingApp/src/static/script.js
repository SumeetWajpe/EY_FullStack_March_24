...
async function DeleteProduct() {
  let res = await fetch("http://localhost:3000/products/delete", {
    method: "DELETE",
  });
  ...
  if (message.status) {
    ...
    // DOM manipulation | reload the page | get the data from the sever (fetch)
  }
}
...
function CreateProductCard(product) {
 ...
 <a href="/products/details/${product.id}"> <img src=${product.imageUrl} class="card-img-top" alt=${product.title} width="200px" height="150px"></a>
  <div class="card-body">
    ...
    <button class="btn btn-danger" onclick="DeleteProduct()"> <i class="fa-solid fa-trash"></i> </button>
    </div>
</div>
</div>
`;
